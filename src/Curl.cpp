                          ...
                          const StringMap<AssnArgData> &assnArgs)
{
    if(!args[1]->is<VarStr>()) {
        vm.fail(args[1]->getLoc(),
                "expected trace config to be of type 'str', found: ", vm.getTypeName(args[1]));
        return nullptr;
    }
                                ...
                                const StringMap<AssnArgData> &assnArgs)
{
    if(!args[1]->is<VarInt>()) {
        vm.fail(args[1]->getLoc(),
                "expected error code to be of type 'int', found: ", vm.getTypeName(args[1]));
        return nullptr;
    }
                               ...
                               const StringMap<AssnArgData> &assnArgs)
{
    ...
    if(!args[1]->is<VarInt>()) {
        vm.fail(loc, "expected an integer (CURL_OPT_*) as parameter for option type, found: ",
                vm.getTypeName(args[1]));
        return nullptr;
    }
    int opt  = as<VarInt>(args[1])->getVal();
    ...
    case CURLOPT_COPYPOSTFIELDS: {
        if(!arg->is<VarStr>()) {
            vm.fail(loc,
                    "expected a string as parameter for this option, found: ", vm.getTypeName(arg));
            return nullptr;
        }
...
void setEnumVars(VirtualMachine &vm, VarModule *mod, ModuleLoc loc)
{
    ...
    mod->addNativeVar("E_WEIRD_SERVER_REPLY", vm.makeVar<VarInt>(loc, CURLE_WEIRD_SERVER_REPLY));
#else
    ...
    mod->addNativeVar("E_FTP_ACCEPT_TIMEOUT", vm.makeVar<VarInt>(loc, CURLE_FTP_ACCEPT_TIMEOUT));
    ...
    mod->addNativeVar("E_HTTP_RETURNED_ERROR", vm.makeVar<VarInt>(loc, CURLE_HTTP_RETURNED_ERROR));
    ...
    mod->addNativeVar("E_OPERATION_TIMEDOUT", vm.makeVar<VarInt>(loc, CURLE_OPERATION_TIMEDOUT));
    ...
    mod->addNativeVar("E_BAD_DOWNLOAD_RESUME", vm.makeVar<VarInt>(loc, CURLE_BAD_DOWNLOAD_RESUME));
    ...
    mod->addNativeVar("E_LDAP_SEARCH_FAILED", vm.makeVar<VarInt>(loc, CURLE_LDAP_SEARCH_FAILED));
    mod->addNativeVar("E_OBSOLETE40", vm.makeVar<VarInt>(loc, CURLE_OBSOLETE40));
    mod->addNativeVar("E_FUNCTION_NOT_FOUND", vm.makeVar<VarInt>(loc, CURLE_FUNCTION_NOT_FOUND));
    mod->addNativeVar("E_ABORTED_BY_CALLBACK", vm.makeVar<VarInt>(loc, CURLE_ABORTED_BY_CALLBACK));
    ...
    mod->addNativeVar("E_TOO_MANY_REDIRECTS", vm.makeVar<VarInt>(loc, CURLE_TOO_MANY_REDIRECTS));
    ...
    mod->addNativeVar("E_SSL_ENGINE_NOTFOUND", vm.makeVar<VarInt>(loc, CURLE_SSL_ENGINE_NOTFOUND));
    ...
    mod->addNativeVar("E_REMOTE_FILE_EXISTS", vm.makeVar<VarInt>(loc, CURLE_REMOTE_FILE_EXISTS));
    ...
    mod->addNativeVar("E_SSL_CACERT_BADFILE", vm.makeVar<VarInt>(loc, CURLE_SSL_CACERT_BADFILE));
    ...
    mod->addNativeVar("E_SSL_SHUTDOWN_FAILED", vm.makeVar<VarInt>(loc, CURLE_SSL_SHUTDOWN_FAILED));
    ...
    mod->addNativeVar("E_RTSP_SESSION_ERROR", vm.makeVar<VarInt>(loc, CURLE_RTSP_SESSION_ERROR));
    ...
    mod->addNativeVar("E_RECURSIVE_API_CALL", vm.makeVar<VarInt>(loc, CURLE_RECURSIVE_API_CALL));
#endif
    ...
    mod->addNativeVar("E_QUIC_CONNECT_ERROR", vm.makeVar<VarInt>(loc, CURLE_QUIC_CONNECT_ERROR));
#endif

    // CURLMcode
    mod->addNativeVar("M_CALL_MULTI_PERFORM", vm.makeVar<VarInt>(loc, CURLM_CALL_MULTI_PERFORM));
...
#if CURL_AT_LEAST_VERSION(7, 59, 0)
    mod->addNativeVar("M_RECURSIVE_API_CALL", vm.makeVar<VarInt>(loc, CURLM_RECURSIVE_API_CALL));
#endif
#if CURL_AT_LEAST_VERSION(7, 68, 0)
    ...
    mod->addNativeVar("UE_UNSUPPORTED_SCHEME", vm.makeVar<VarInt>(loc, CURLUE_UNSUPPORTED_SCHEME));
    ...
    mod->addNativeVar("OPT_XFERINFOFUNCTION", vm.makeVar<VarInt>(loc, CURLOPT_XFERINFOFUNCTION));
    ...
    mod->addNativeVar("OPT_SSL_CTX_FUNCTION", vm.makeVar<VarInt>(loc, CURLOPT_SSL_CTX_FUNCTION));
    ...
    mod->addNativeVar("OPT_FNMATCH_FUNCTION", vm.makeVar<VarInt>(loc, CURLOPT_FNMATCH_FUNCTION));
    ...
    mod->addNativeVar("OPT_DEFAULT_PROTOCOL", vm.makeVar<VarInt>(loc, CURLOPT_DEFAULT_PROTOCOL));
...
#endif
    mod->addNativeVar("OPT_SOCKS5_GSSAPI_NEC", vm.makeVar<VarInt>(loc, CURLOPT_SOCKS5_GSSAPI_NEC));
    ...
    mod->addNativeVar("OPT_DNS_CACHE_TIMEOUT", vm.makeVar<VarInt>(loc, CURLOPT_DNS_CACHE_TIMEOUT));
#if CURL_AT_LEAST_VERSION(7, 62, 0)
    ...
    mod->addNativeVar("OPT_UNIX_SOCKET_PATH", vm.makeVar<VarInt>(loc, CURLOPT_UNIX_SOCKET_PATH));
    ...
    mod->addNativeVar("OPT_TLSAUTH_USERNAME", vm.makeVar<VarInt>(loc, CURLOPT_TLSAUTH_USERNAME));
    mod->addNativeVar("OPT_TLSAUTH_PASSWORD", vm.makeVar<VarInt>(loc, CURLOPT_TLSAUTH_PASSWORD));
    ...
    mod->addNativeVar("OPT_TRANSFER_ENCODING", vm.makeVar<VarInt>(loc, CURLOPT_TRANSFER_ENCODING));
    mod->addNativeVar("OPT_FOLLOWLOCATION", vm.makeVar<VarInt>(loc, CURLOPT_FOLLOWLOCATION));
    mod->addNativeVar("OPT_UNRESTRICTED_AUTH", vm.makeVar<VarInt>(loc, CURLOPT_UNRESTRICTED_AUTH));
    ...
    mod->addNativeVar("OPT_STREAM_DEPENDS_E", vm.makeVar<VarInt>(loc, CURLOPT_STREAM_DEPENDS_E));
    ...
    mod->addNativeVar("OPT_FTP_SKIP_PASV_IP", vm.makeVar<VarInt>(loc, CURLOPT_FTP_SKIP_PASV_IP));
    ...
    mod->addNativeVar("OPT_RTSP_CLIENT_CSEQ", vm.makeVar<VarInt>(loc, CURLOPT_RTSP_CLIENT_CSEQ));
    mod->addNativeVar("OPT_RTSP_SERVER_CSEQ", vm.makeVar<VarInt>(loc, CURLOPT_RTSP_SERVER_CSEQ));
    ...
    mod->addNativeVar("OPT_RESUME_FROM_LARGE", vm.makeVar<VarInt>(loc, CURLOPT_RESUME_FROM_LARGE));
    ...
    mod->addNativeVar("OPT_INFILESIZE_LARGE", vm.makeVar<VarInt>(loc, CURLOPT_INFILESIZE_LARGE));
    mod->addNativeVar("OPT_UPLOAD", vm.makeVar<VarInt>(loc, CURLOPT_UPLOAD));
#if CURL_AT_LEAST_VERSION(7, 62, 0)
    mod->addNativeVar("OPT_UPLOAD_BUFFERSIZE", vm.makeVar<VarInt>(loc, CURLOPT_UPLOAD_BUFFERSIZE));
#endif
    ...
    mod->addNativeVar("OPT_MAXFILESIZE_LARGE", vm.makeVar<VarInt>(loc, CURLOPT_MAXFILESIZE_LARGE));
    ...
    mod->addNativeVar("OPT_CONNECTTIMEOUT_MS", vm.makeVar<VarInt>(loc, CURLOPT_CONNECTTIMEOUT_MS));
...
#endif
    mod->addNativeVar("OPT_ACCEPTTIMEOUT_MS", vm.makeVar<VarInt>(loc, CURLOPT_ACCEPTTIMEOUT_MS));
#if CURL_AT_LEAST_VERSION(7, 59, 0)
    ...
    mod->addNativeVar("OPT_SSLENGINE_DEFAULT", vm.makeVar<VarInt>(loc, CURLOPT_SSLENGINE_DEFAULT));
    ...
    mod->addNativeVar("OPT_SSL_VERIFYSTATUS", vm.makeVar<VarInt>(loc, CURLOPT_SSL_VERIFYSTATUS));
#if CURL_AT_LEAST_VERSION(7, 52, 0)
    ...
    mod->addNativeVar("OPT_PROXY_SSLCERTTYPE", vm.makeVar<VarInt>(loc, CURLOPT_PROXY_SSLCERTTYPE));
    mod->addNativeVar("OPT_PROXY_SSLKEY", vm.makeVar<VarInt>(loc, CURLOPT_PROXY_SSLKEY));
    mod->addNativeVar("OPT_PROXY_SSLKEYTYPE", vm.makeVar<VarInt>(loc, CURLOPT_PROXY_SSLKEYTYPE));
    mod->addNativeVar("OPT_PROXY_SSLVERSION", vm.makeVar<VarInt>(loc, CURLOPT_PROXY_SSLVERSION));
    mod->addNativeVar("OPT_PROXY_SSL_CIPHER_LIST",
                      vm.makeVar<VarInt>(loc, CURLOPT_PROXY_SSL_CIPHER_LIST));
    mod->addNativeVar("OPT_PROXY_SSL_OPTIONS", vm.makeVar<VarInt>(loc, CURLOPT_PROXY_SSL_OPTIONS));
    ...
    mod->addNativeVar("OPT_GSSAPI_DELEGATION", vm.makeVar<VarInt>(loc, CURLOPT_GSSAPI_DELEGATION));
    ...
    mod->addNativeVar("INFO_NAMELOOKUP_TIME", vm.makeVar<VarInt>(loc, CURLINFO_NAMELOOKUP_TIME));
    mod->addNativeVar("INFO_CONNECT_TIME", vm.makeVar<VarInt>(loc, CURLINFO_CONNECT_TIME));
    mod->addNativeVar("INFO_PRETRANSFER_TIME", vm.makeVar<VarInt>(loc, CURLINFO_PRETRANSFER_TIME));
    mod->addNativeVar("INFO_SIZE_UPLOAD_T", vm.makeVar<VarInt>(loc, CURLINFO_SIZE_UPLOAD_T));
    mod->addNativeVar("INFO_SIZE_DOWNLOAD_T", vm.makeVar<VarInt>(loc, CURLINFO_SIZE_DOWNLOAD_T));
    mod->addNativeVar("INFO_SPEED_DOWNLOAD_T", vm.makeVar<VarInt>(loc, CURLINFO_SPEED_DOWNLOAD_T));
    ...
    mod->addNativeVar("INFO_SSL_VERIFYRESULT", vm.makeVar<VarInt>(loc, CURLINFO_SSL_VERIFYRESULT));
    ...
    mod->addNativeVar("INFO_HTTP_CONNECTCODE", vm.makeVar<VarInt>(loc, CURLINFO_HTTP_CONNECTCODE));
    mod->addNativeVar("INFO_HTTPAUTH_AVAIL", vm.makeVar<VarInt>(loc, CURLINFO_HTTPAUTH_AVAIL));
    mod->addNativeVar("INFO_PROXYAUTH_AVAIL", vm.makeVar<VarInt>(loc, CURLINFO_PROXYAUTH_AVAIL));
    ...
    mod->addNativeVar("INFO_APPCONNECT_TIME", vm.makeVar<VarInt>(loc, CURLINFO_APPCONNECT_TIME));
    mod->addNativeVar("INFO_CERTINFO", vm.makeVar<VarInt>(loc, CURLINFO_CERTINFO));
    mod->addNativeVar("INFO_CONDITION_UNMET", vm.makeVar<VarInt>(loc, CURLINFO_CONDITION_UNMET));
    mod->addNativeVar("INFO_RTSP_SESSION_ID", vm.makeVar<VarInt>(loc, CURLINFO_RTSP_SESSION_ID));
    mod->addNativeVar("INFO_RTSP_CLIENT_CSEQ", vm.makeVar<VarInt>(loc, CURLINFO_RTSP_CLIENT_CSEQ));
    mod->addNativeVar("INFO_RTSP_SERVER_CSEQ", vm.makeVar<VarInt>(loc, CURLINFO_RTSP_SERVER_CSEQ));
    ...
    mod->addNativeVar("INFO_REDIRECT_TIME_T", vm.makeVar<VarInt>(loc, CURLINFO_REDIRECT_TIME_T));
    ...
    mod->addNativeVar("INFO_EFFECTIVE_METHOD", vm.makeVar<VarInt>(loc, CURLINFO_EFFECTIVE_METHOD));
    ...
    mod->addNativeVar("INFO_EARLYDATA_SENT_T", vm.makeVar<VarInt>(loc, CURLINFO_EARLYDATA_SENT_T));
